...
test("production audit defaults to the released zamaanak Vercel origin and requires HTTPS", async () => {
  const source = await read("scripts/remote-production-audit.mjs");
  assert.match(source, /DEFAULT_PRODUCTION_URL = "https:\/\/zamaanak\.ir\/"/);
  assert.match(source, /Production audit requires HTTPS/);
  assert.match(source, /zamaanak_PRODUCTION_URL/);
});
