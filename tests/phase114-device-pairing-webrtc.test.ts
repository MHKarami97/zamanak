...
test("pairing links keep the bearer secret in the URL fragment", () => {
  ...
  const link = createPairingLink(code, "https://zamaanak.example");
  assert.match(link, /^https:\/\/zamaanak\.example\/settings#device-pair=/);
...
test("WebRTC pairing is dependency-free and uses a direct ordered data channel", () => {
  ...
  assert.match(peer, /createDataChannel\("zamaanak-transfer", \{ ordered: true \}\)/);
