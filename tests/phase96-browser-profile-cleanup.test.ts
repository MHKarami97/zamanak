...
test("browser profile cleanup retries transient Windows locks", async () => {
  let calls = 0;
  const delays: number[] = [];
  const result = await removeBrowserProfileDirectory("C:\\Temp\\zamaanak-profile", {
    attempts: 4,
    retryDelayMs: 10,
    remove: async (directory, options) => {
      calls += 1;
      assert.equal(directory, "C:\\Temp\\zamaanak-profile");
...
test("browser profile cleanup preserves non-transient filesystem failures", async () => {
  await assert.rejects(
    removeBrowserProfileDirectory("C:\\Temp\\zamaanak-profile", {
      remove: async () => {
...
test("best-effort smoke cleanup reports a lingering lock without failing the run", async () => {
  ...
  let calls = 0;
  const result = await cleanupBrowserProfile("C:\\Temp\\zamaanak-profile", {
    attempts: 3,
    retryDelayMs: 0,
    remove: async () => {
      calls += 1;
