...
export function PwaExperience() {
  ...
  if (!online) {
    return (
      <PwaBanner>
        <AlertBanner tone="warning" icon={<WifiOff />} title="زمانک آفلاین است">
    ...
    return (
      <PwaBanner>
        <AlertBanner
          tone="info"
          icon={<RefreshCw />}
          title="نسخه جدید زمانک آماده است"
          action={<Button size="sm" onClick={update} disabled={updating}>{updating ? "در حال به‌روزرسانی…" : "به‌روزرسانی امن"}</Button>}
        >
          با تأیید تو نسخه جدید فعال می‌شود. اگر Draft ذخیره‌نشده داشته باشی، زمانک قبل از Reload هشدار می‌دهد.
        </AlertBanner>
      </PwaBanner>
    );
  }
    ...
    return (
      <PwaBanner>
        <AlertBanner
          tone="success"
          icon={<Download />}
          title="زمانک را مثل یک اپ نصب کن"
          action={
    ...
    return (
      <PwaBanner>
        <AlertBanner
          tone="info"
          icon={<Share2 />}
          title="نصب زمانک روی iPhone یا iPad"
        ...
        >
          از منوی Share مرورگر، گزینه Add to Home Screen را انتخاب کن تا زمانک مثل یک اپ اجرا شود.
        </AlertBanner>
      </PwaBanner>
    );
  }
