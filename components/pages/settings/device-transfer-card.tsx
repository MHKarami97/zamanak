...
export function DeviceTransferCard({ data, setData, setToast }: {
  ...
  const shareLocalLink = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: "اتصال زمانک", text: "این لینک Pairing فقط برای همین نشست است.", url: pairing.pairingLink });
        return;
      } catch { /* copy fallback */ }
    }
    ...
    <section id="settings-device-transfer" className="scroll-mt-24 dashboard-card rounded-[var(--card-radius)] border border-[var(--dashboard-border)] p-5 shadow-[0_5px_16px_rgba(0,0,0,.03)]">
      ...
      {(pairing.role === "idle" || pairing.role === "receiver") && (
        <div className="mt-4">
          ...
          <Textarea value={pairing.remoteCode} onChange={(event) => pairing.setRemoteCode(event.target.value)} placeholder="zamaanak-pair:... یا لینک زمانک" className="mt-2 min-h-24 break-all text-left text-[10px]" dir="ltr" />
