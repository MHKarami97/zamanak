...
export function BackupImportPanel({ data, commitImport }: {
  ...
  async function onFile(file?: File) {
    ...
    try {
      ...
      setError("فایل انتخاب‌شده یک پشتیبان معتبر زمانک نیست.");
    }
  }
  ...
  return (
    <section className="dashboard-card rounded-[var(--card-radius)] border border-[var(--dashboard-border)] p-4 sm:p-5">
      <PanelHead icon={<DatabaseBackup />} title="واردسازی پشتیبان زمانک" />
      ...
      <FileDropField accept=".json,application/json" title="فایل JSON زمانک را انتخاب کن" description="پشتیبان نسخه‌های قبلی هم در صورت پشتیبانی Schema مهاجرت داده می‌شود" onFile={(file) => { void onFile(file); }} />
