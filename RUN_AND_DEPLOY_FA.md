# راهنمای اجرا و انتشار زمانک

این راهنما برای نسخه نهایی زمانک است. پروژه Backend و دیتابیس سروری ندارد،
...
Set-Location "D:\my-workspace\zamaanak-worklog"
...
```bat
cd /d D:\my-workspace\zamaanak-worklog
node --version
npm.cmd ci
npm.cmd run check
npm.cmd run dev
...
Docker برای زمانک لازم نیست، اما در صورت نیاز:
...
```bash
docker build -t zamaanak .
docker run --rm -p 8080:80 zamaanak
```
...
```powershell
$env:zamaanak_BROWSER_PATH = "C:\Program Files\Google\Chrome\Application\chrome.exe"
npm.cmd run test:browser:production
```
