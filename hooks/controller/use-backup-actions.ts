...
export function useBackupActions({ data, setData, setToast, importPreview, setImportPreview, storage }: Args) {
  function exportBackup() { downloadBlob(backupBlob(data), `zamaanak-backup-${localDateKey()}.json`); setToast("فایل پشتیبان دانلود شد"); }
  ...
  async function commitImport(next: AppData, message: string, options: { safetyBackup?: boolean } = {}) {
    if (options.safetyBackup) downloadBlob(backupBlob(data), `zamaanak-before-import-${localDateKey()}.json`);
    try {
