...
zamaanak remains local-first. Device transfer does not add a permanent backend or central database. Pairing codes are short-lived bearer secrets and must remain private; AppData itself is transferred over the established AES-GCM encrypted session.
...
```bash
git tag -a v2.2.0 -m "zamaanak 2.2.0"
