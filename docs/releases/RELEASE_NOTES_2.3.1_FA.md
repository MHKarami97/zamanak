...
- دستور `npm run audit:production` مسیرهای عمومی، App Shell فارسی/RTL، PWA Manifest، Service Worker، Precache، آیکن‌های نصب، robots و sitemap را روی `https://zamaanak.ir/` بررسی می‌کند.
- Parser Precache با فرمت واقعی `self.__zamaanak_PRECACHE` و مسیرهای نسبی `_next/static/...` هماهنگ شد تا false negative قبلی حذف شود.
