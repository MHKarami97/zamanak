...
- [x] فاز ۱۵۵: Audit پس از انتشار روی دامنه واقعی `https://zamaanak.ir/`؛ بررسی read-only Routeهای عمومی، Settings، PWA Manifest/Service Worker/Precache، آیکون‌ها، robots و sitemap بدون تغییر Manifest تاریخی 2.3.0.
- [x] فاز ۱۵۶: اصلاح false negative Audit Precache Production؛ parse مستقیم `self.__zamaanak_PRECACHE`، پشتیبانی از مسیر نسبی `_next/static/...` و تأیید reachability اولین Build Asset.
