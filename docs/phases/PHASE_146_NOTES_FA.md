...
بررسی DOM contract نشان داد `Checkbox` مشترک زمانک یک `input[type="checkbox"]` واقعی است. Smoke هنوز قرارداد قدیمی Radix (`role="checkbox"` + `data-state`/`aria-checked`) را جست‌وجو می‌کرد؛ بنابراین کنترل موجود در UI را نمی‌دید.
