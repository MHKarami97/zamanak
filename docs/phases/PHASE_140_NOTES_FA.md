...
Persistence واقعی زمانک از `AppDataStorageAdapter` استفاده می‌کند و پس از اولین save مقدار Store را به شکل snapshot envelope ذخیره می‌کند:

```text
{
  format: "zamaanak-app-data",
  schemaVersion: 17,
  savedAt: "...",
  data: { ...AppData }
}
...
- AppData Schema: v17
- snapshot envelope: `zamaanak-app-data`
