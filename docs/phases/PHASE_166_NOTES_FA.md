...
جداکردن راه‌اندازی اولیه زمانک از صفحه `Today` و تبدیل آن به Route واقعی `/onboarding`، بدون تغییر قرارداد داده، Schema یا محتوای مراحل فعلی آنبوردینگ.

در نسخه‌های قبلی، Component آنبوردینگ به‌صورت Overlay سراسری داخل `zamaanakShell` رندر می‌شد. در نتیجه کاربر عملاً روی Route صفحه امروز قرار داشت، هرچند فقط آنبوردینگ را می‌دید. این فاز مسئولیت Route و App Shell را شفاف می‌کند.
...
2. مرحله ورود داده برای کاربرانی که قبلاً در ابزار دیگری ساعت، پروژه یا اطلاعات مالی داشته‌اند؛ ابتدا Backup زمانک و سپس Importهای ساخت‌یافته مثل CSV با Preview قبل از اعمال.
