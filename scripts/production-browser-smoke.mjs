...
export function browserExecutableCandidates(env = process.env, platform = process.platform) {
  const overrides = [env.zamaanak_BROWSER_PATH, env.PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH, env.CHROME_PATH]
...
async function readStoredSettings(client) {
  return evaluate(client, `(async () => {
    const db = await new Promise((resolve, reject) => {
      const request = indexedDB.open("zamaanak-db", 1);
    ...
    const data = stored?.format === "zamaanak-app-data" && stored?.data ? stored.data : stored;
...
export async function runProductionBrowserSmoke() {
  ...
  if (!browserExecutable) {
    throw new Error("Chrome, Edge or Chromium was not found. Set zamaanak_BROWSER_PATH to the browser executable.");
  }

  const debugPort = await freePort();
  const profileDir = await mkdtemp(join(tmpdir(), "zamaanak-browser-smoke-"));
  let staticServer;
  let browser;
  let client;
  let browserOutput = "";
  ...
  try {
    ...
    await waitFor(client, `(async () => {
      const db = await new Promise((resolve, reject) => { const request = indexedDB.open("zamaanak-db", 1); request.onsuccess = () => resolve(request.result); request.onerror = () => reject(request.error); });
      ...
      const appData = stored?.format === "zamaanak-app-data" && stored?.data ? stored.data : stored;
    ...
    await waitFor(client, "['/today', '/today/'].includes(location.pathname) && !document.body?.innerText.includes('شروع زمانک')", "today route after onboarding");
    ...
    await returnToday;
    await waitFor(client, `["/today", "/today/"].includes(location.pathname) && document.body?.innerText.includes("زمانک")`, "Today after Import Wizard");
    ...
    if (!firstInstallWorker.controlled) {
      ...
      await waitFor(client, "document.readyState === 'complete' && document.body?.innerText.includes('زمانک')", "first-install PWA control reload render", 45_000);
    }
    ...
    if (!pwaContract?.active || !pwaContract.controlled || pwaContract.name !== "زمانک" || pwaContract.shortName !== "زمانک" || pwaContract.display !== "standalone" || pwaContract.iconCount < 3 || pwaContract.precachedBuildAssets < 1) {
    ...
    await client.call("Page.reload", { ignoreCache: false });
    await offlineLoad;
    await waitFor(client, "document.readyState === 'complete' && document.body?.innerText.includes('زمانک')", "offline PWA reload", 45_000);
