...
export const DEFAULT_PRODUCTION_URL = "https://zamaanak.ir/";
...
async function request(url, { binary = false } = {}) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: { "user-agent": "zamaanak-Production-Audit/1.0" },
    });
...
function assertHtmlContract(path, html) {
  ...
  if (!html.includes("زمانک")) throw new Error(`${path} does not expose the zamaanak product identity.`);
...
function assertManifestContract(manifest) {
  if (manifest?.name !== "زمانک" || manifest?.short_name !== "زمانک") {
...
export function parsePrecacheEntries(source) {
  const match = source.match(/self\.__zamaanak_PRECACHE\s*=\s*(\[[\s\S]*?\])\s*;/);
  if (!match) throw new Error("PWA precache manifest does not expose self.__zamaanak_PRECACHE.");
  let entries;
  try {
...
export async function runRemoteProductionAudit(inputUrl = process.env.zamaanak_PRODUCTION_URL || DEFAULT_PRODUCTION_URL) {
  const base = normalizeBaseUrl(inputUrl);
  const origin = base.origin;
  console.log(`zamaanak production audit: ${base.toString()}`);
  ...
  console.log("✓ PWA manifest exposes zamaanak standalone/RTL install metadata");

  const swResult = await request(sameOriginPath(base, "/sw.js"));
  assertOk("Service worker", swResult.response, origin);
  if (!/importScripts\(["']pwa-precache-manifest\.js["']\)/.test(swResult.body) || !/zamaanak-shell-v\d+/.test(swResult.body)) {
    throw new Error("Service worker does not expose the expected zamaanak cache/precache contract.");
  }
...
if (isDirectRun) {
  const inputUrl = process.argv[2] || process.env.zamaanak_PRODUCTION_URL || DEFAULT_PRODUCTION_URL;
