...
export function collectReleaseAuditFailures() {
  ...
  requireCondition(includesLine("RELEASE_CHECKLIST_FA.md", `# چک‌لیست انتشار زمانک ${manifest.version}`), "Release checklist version is stale.", failures);
...
export function runReleaseAudit() {
  ...
  if (failures.length > 0) {
    console.error("zamaanak release audit failed\n");
  ...
  const manifest = readJson(RELEASE_MANIFEST_PATH);
  console.log(`zamaanak ${manifest.version} release audit passed.`);
