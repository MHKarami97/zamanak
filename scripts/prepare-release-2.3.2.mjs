...
const baseline = existingIsCommit
  ? prepared
  : process.env.zamaanak_RELEASE_BASELINE?.trim() || execFileSync("git", ["rev-parse", "--short=7", "HEAD"], { cwd: root, encoding: "utf8" }).trim();
...
console.log(existingIsCommit
  ? `zamaanak 2.3.2 baseline already prepared: ${baseline}`
  : `zamaanak 2.3.2 baseline captured from HEAD: ${baseline}`);
