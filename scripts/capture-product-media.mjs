...
async function seedAppData(client, data) {
  await evaluate(client, `(async () => {
    ...
    db.close();
    localStorage.setItem("zamaanak:last-route", "/today");
    return true;
  })()`);
}

async function setTheme(client, mode) {
  await evaluate(client, `localStorage.setItem("zamaanak-appearance", JSON.stringify({mode:${JSON.stringify(mode)},preset:"spotify",accent:"#06b6d4",radius:"rounded",surface:"tinted"}));`);
}
...
async function captureOnboardingFrames(client, origin) {
  await navigate(client, origin, "به زمانک خوش آمدی");
  ...
  for (let index = 1; index <= 7; index += 1) {
    ...
    const label = index < 7 ? "ادامه" : "شروع زمانک";
...
async function main() {
  ...
  if (!browserExecutable) throw new Error("Chrome, Edge or Chromium was not found. Set zamaanak_BROWSER_PATH.");

  await mkdir(SCREENSHOT_DIR, { recursive: true });
  await mkdir(MEDIA_DIR, { recursive: true });
  const profileDir = await mkdtemp(join(tmpdir(), "zamaanak-media-"));
  ...
  try {
    ...
    await client.call("Storage.clearDataForOrigin", { origin: server.origin, storageTypes: "all" });
    await navigate(client, server.origin, "زمانک را برای خودت تنظیم کن");
